/**
 * Coder: Karthik Rachamolla
 * Last updated: 8/7/2025
 * Project: Trainer Diagnostic Interface
 * Note: Gateway designed to interact with trainer/car ECU to request DTCs, reset DTCs, and read live data
 * The code below is the config and setup for BLE based communication with the web frontend.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// Initial value for control characteristic
...
// Client Characteristic Configuration Descriptor initial value
...
// serializes writes from BLE client to UI handler
...
// Send entire bitmask to the BLE client
static void ble_send_pid_mask(void) {
    ESP_LOGI(TAG, "ble_send_pid_mask()");
    // Ask ECU for the full 7×4 bitmask
...
// Send single PID value to the BLE client
static void ble_send_pid_value(uint8_t pid) {
    ...
    // Build a BLE packet: [pid, data...] to sent to client
    uint8_t pkt[len + 1];
    pkt[0] = pid;
...
/** Continuous streaming - DEV IN PROGRESS */
...
// Handle incoming BLE commands from the UI
static void handle_ui_cmd(const uint8_t *buf, uint16_t len) {
    ...
    switch (buf[0]) {
        case CMD_DTC_PENDING:
        case CMD_DTC_STORED:
        case CMD_DTC_PERM: {
            // Common write handler for Pending, stored and PERM.
            service_request_t svc;
            if  (buf[0] == CMD_DTC_PENDING) svc = SERV_PENDING_DTCS;
            else if (buf[0] == CMD_DTC_STORED) svc = SERV_STORED_DTCS;
            else svc = SERV_PERM_DTCS;

            ESP_LOGI(TAG, "DTC request over BLE: cmd=0x%02X -> service %d", buf[0], svc);
            int rc  = Set_TWAI_Serv(svc);
            if (rc != 0) {
                ESP_LOGW(TAG, "TWAI DTC request error %d", rc);
            }

            // grab the DTC array out of TC_ref
            uint8_t *dtcs = get_dtcs();
            uint8_t dtc_len = get_dtcs_bytes();

            ESP_LOGI(TAG, "Read %d bytes of DTC data", dtc_len);
            BLE_push_dtcs(dtcs, dtc_len);
            break;
        }

        // Clear DTC handler
        case CMD_DTC_CLEAR: {
            ESP_LOGI(TAG, "Clear DTCs command");
            Set_TWAI_Serv(SERV_CLEAR_DTCS);
            BLE_notify_clear();
            break;
        }
        
        // aka send bitmask to frontend
        case CMD_STATUS:
        ...
        // Get PID values for every other "cmd"
        default:
            for (int i = 0; i < len; i++) {
                uint8_t pid = buf[i];
...
// Push DTC Codes to Client in <20 byte chunks
